...
lrnm_laplace.init = function(X, B = ilr_basis(ncol(X))){
  ...
  while(iter < 1000){
    ...
    eig = eigen(cov_)
    # print(eig$values)
    eig_values = quantile(eig$values, 0.1) / min(eig$values) * eig$values # quantile(eig$values, 0.5)/d + eig$values #pmax(eig$values, quantile(eig$values, 0.75))
    S = eig$vectors %*% diag(eig_values)  %*% t(eig$vectors)
    
    # Posterior laplace approximation
    A = lapply(split(X,seq(NROW(X))), c_posterior_approximation_vec, mu_, solve(S), Binv)
